var EncontraGanhador = function () {
    ...
    var encontraGanhador = function() {
        ...
        $.get(window.api_url + 'cupons/encontra_ganhador/?token=' + window.user_token + '&usuario=' + window.user_email + '&campanha_id=' + window.campanha_id + '&sorteio_id=' + window.sorteio_id + '&numero_sorteado=' + window.numero_sorteado,{},function(data){
			if ( data.status != 'ok' ) {
				Swal.fire({
					icon: 'error',
					title: 'Oops...',
